...
//  EssentialApp
//
//  Created by Станислав Соколов on 3/24/26.
//
...
extension Publisher {
    func dispatchOnMainThread() -> AnyPublisher<Output, Failure> {
        receive(on: DispatchQueue.immediateWhenOnMainThreadSchedule).eraseToAnyPublisher()
    }
}

extension DispatchQueue {

    static var immediateWhenOnMainQueueSchedule: ImmediateWhenOnMainQueueSchedule {
        ImmediateWhenOnMainQueueSchedule.shared
    }

    struct ImmediateWhenOnMainQueueSchedule: Scheduler {
    ...
    static var immediateWhenOnMainThreadSchedule: ImmediateWhenOnMainThreadSchedule {
        ImmediateWhenOnMainThreadSchedule()
    }

    struct ImmediateWhenOnMainThreadSchedule: Scheduler {
        typealias SchedulerTimeType = DispatchQueue.SchedulerTimeType
        typealias SchedulerOptions = DispatchQueue.SchedulerOptions
...
extension AnyDispatchQueueScheduler {
    static var immediateOnMainQueue: Self {
        DispatchQueue.immediateWhenOnMainQueueSchedule.eraseToAnyScheduler()
    }
